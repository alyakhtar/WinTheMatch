...
def player_stats(player, i ):
    ...
    with con:
        cur = con.cursor()
        cur.execute("CREATE TABLE IF NOT EXISTS bowling_statistics(Id INT PRIMARY KEY AUTO_INCREMENT, Overs VARCHAR(25) , Maidens VARCHAR(25), Runs VARCHAR(25), Wickets VARCHAR(25), Eco VARCHAR(25), Pos VARCHAR(25), Inns VARCHAR(25), Ground VARCHAR(40), Start_Date VARCHAR(25), ODI_NO VARCHAR(25), Opposition VARCHAR(30), Result VARCHAR(140), Player VARCHAR(30), Type VARCHAR(30))")

        for row in mt:
            cur.execute("""INSERT INTO bowling_statistics (Overs,Maidens,Runs,Wickets,Eco,Pos,Inns,Ground,Start_Date,ODI_NO,Opposition,Result,Player,Type) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)""", (row[0],row[1],row[2],row[3],row[4],row[5],row[6],row[7],row[8],row[9],row[10],row[11],row[12],row[13]))
